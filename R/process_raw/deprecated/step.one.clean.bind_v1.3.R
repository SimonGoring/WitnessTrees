...
#  Assumes you're in a working directory that contains the folder 'Maps':
  ifelse(.Platform$OS.type=='unix',
         ...
         setwd('C:/Users/goring/Dropbox/PalEON/WitnessTreeDensity'))

pot.veg <- raster(read.asciigrid(fname='Maps/glpotveg_5min.asc'))

wisc <- readOGR('Maps/glo_corn.shp', 'glo_corn')
minn <- readOGR('Maps/Minnesota.shp', 'Minnesota')
mich <- readOGR('Maps/michigan.shp', 'michigan')

#  The files are in unique projections, this standardizes the projections to
#  match the potential vegetation maps.
...
#  This file contains several functions that wind up getting used throughout.
...
azimuths <- apply(azimuths, 2, getAngle)

#####  Cleaning Trees:  Changing tree codes to lumped names:
#  The file witnesstreecodesv4.csv contains a key that links all the two letter
#  codes from Wisconsin and Minnesota to established, and taxonomically sensible
#  species groups.  It also includes 'Water' and 'NonTree' codes.
spec.codes <- read.csv('fullpaleon_conversion_v0.3.csv', stringsAsFactor = FALSE)
#spec.codes <- read.csv('conversion_table.csv', stringsAsFactor = FALSE)

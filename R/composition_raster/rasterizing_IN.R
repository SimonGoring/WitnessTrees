...
indiana <- read.csv('data/raw_data/indiana/ndinpls_v1.4.csv')

#  We need to check to make sure the closest two trees are the first two.

#  Clear leading and trailing spaces.
indiana$L1_tree1 <- gsub('^[ \t]+|[ \t]+$', '', indiana$L1_tree1)
indiana$L1_tree1 <- gsub('[ ]{2}', ' ', indiana$L1_tree1)
indiana$L1_tree2 <- gsub('^[ \t]+|[ \t]+$', '', indiana$L1_tree2)
indiana$L1_tree2 <- gsub('[ ]{2}', ' ', indiana$L1_tree2)
...
indiana$l3a.tree1 <-  taxon.conv[match(tolower(indiana$L1_tree1), 
                                       tolower(taxon.conv$Level.1)),6]

indiana$l3a.tree2 <-  taxon.conv[match(tolower(indiana$L1_tree2), 
                                       tolower(taxon.conv$Level.1)),6]

coordinates(indiana) <- ~ x + y

#proj4string(indiana) <- CRS('+init=epsg:4326')
proj4string(indiana) <- CRS('+init=epsg:3175')
...
write.csv(rast.out, 'data/output/gridded/IndianaData_v0.3.csv')
